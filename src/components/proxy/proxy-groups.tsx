...
export const ProxyGroups = (props: Props) => {
  ...
  const lastPersistedTopRef = useRef<number | null>(null);
  ...
  const persistScrollPositions = useCallback(() => {
    try {
      ...
      lastPersistedTopRef.current = scrollPositionRef.current[mode] ?? null;
    } catch (e) {
      console.error("Error saving scroll position:", e);
    }
  }, [mode]);

  const schedulePersistScrollPositions = useCallback(() => {
  ...
  const flushPersistScrollPositions = useCallback(() => {
  ...
  // 改为批量写入 localStorage，降低滚动时主线程 IO 压力
  const saveScrollPosition = useCallback(
    (scrollTop: number) => {
      scrollPositionRef.current[mode] = scrollTop;

      const lastPersistedTop = lastPersistedTopRef.current;
      if (
        lastPersistedTop !== null &&
        Math.abs(lastPersistedTop - scrollTop) < 12
      ) {
        return;
      }
    ...
    [mode, schedulePersistScrollPositions],
  );
