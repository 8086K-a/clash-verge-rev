...
const ConnectionsPage = () => {
  ...
  const deferredConnections = useDeferredValue(sourceConnections);
  const hasSearchText = searchState.text.trim().length > 0;

  const [isColumnManagerOpen, setIsColumnManagerOpen] = useState(false);

  const filterConn = useMemo(() => {
    const orderFunc = orderFunctionMap[curOrderOpt];
    const conns = deferredConnections ?? [];
    const matchConns = !hasSearchText
      ? conns
      : conns.filter((conn) => {
    ...
    if (isTableLayout || !orderFunc) return matchConns;

    return orderFunc(matchConns);
  }, [deferredConnections, hasSearchText, isTableLayout, match, curOrderOpt]);
